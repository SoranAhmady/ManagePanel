        ...
        {open && (
          ...
          >
            {ArayUser.map((item, index) => (
              <motion.div
                onClick={() => (item.id === 5 && HandelLogUot())}
                initial={{ opacity: 0, y: -30, h: 0 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.5 }}
                whileHover={{ backgroundColor: "#E5F2E9" }}
                className="w-[90%]  border-b-[1px]  border-[#EDEDED] flex items-center justify-end px-[0.5vw] py-[0.48vw] gap-1 rounded-sm   "
              >
